...
__device__ void shellSortWindow(unsigned char* window, int windowSize) {
    int n = windowSize * windowSize;
    for (int gap = n / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; i += 1) {
            unsigned char temp = window[i];
            int j;
            for (j = i; j >= gap && window[j - gap] > temp; j -= gap) {
                window[j] = window[j - gap];
            }
            window[j] = temp;
        }
    }
...
__global__ void medianFilterSharedKernel(unsigned char* __restrict__ input, unsigned char* __restrict__ output, int width, int height) {
    ...
    if (x < width && y < height) {
        ...
        shellSortWindow(window, WINDOW_SIZE);
