...
__device__ int partition(unsigned char* arr, int left, int right, int pivotIndex) {
...
// Función quickselect para encontrar la mediana
__device__ unsigned char quickselect(unsigned char* arr, int left, int right, int k) {
    if (left == right) {
        return arr[left];
    }
...
// Kernel para aplicar el filtro de mediana usando memoria compartida y quickselect
template <int BLOCK_DIM_X, int BLOCK_DIM_Y, int WINDOW_SIZE>
__global__ void medianFilterQuickselectKernel(unsigned char* input, unsigned char* output, int width, int height) {
    ...
    if (x < width && y < height) {
        ...
        int medianIndex = (WINDOW_SIZE * WINDOW_SIZE) / 2;
        output[y * width + x] = quickselect(window, 0, WINDOW_SIZE * WINDOW_SIZE - 1, medianIndex);
    }
}
...
float applyMedianFilterGPU(const PGMImage& input, PGMImage& output, int windowSize) {
    ...
    switch (windowSize) {
        case 3:
            medianFilterQuickselectKernel<BLOCK_DIM_X, BLOCK_DIM_Y, 3><<<gridSize, blockSize>>>(d_input, d_output, input.width, input.height);
            break;
        case 5:
            medianFilterQuickselectKernel<BLOCK_DIM_X, BLOCK_DIM_Y, 5><<<gridSize, blockSize>>>(d_input, d_output, input.width, input.height);
            break;
        case 7:
            medianFilterQuickselectKernel<BLOCK_DIM_X, BLOCK_DIM_Y, 7><<<gridSize, blockSize>>>(d_input, d_output, input.width, input.height);
            break;
        case 9:
            medianFilterQuickselectKernel<BLOCK_DIM_X, BLOCK_DIM_Y, 9><<<gridSize, blockSize>>>(d_input, d_output, input.width, input.height);
            break;
        case 11:
            medianFilterQuickselectKernel<BLOCK_DIM_X, BLOCK_DIM_Y, 11><<<gridSize, blockSize>>>(d_input, d_output, input.width, input.height);
            break;
        default:
