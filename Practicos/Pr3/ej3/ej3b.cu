...
__global__ void kernel(int *A, int *v, int *x) {
    __shared__ int shared_v[N];
    int row = blockIdx.x * blockDim.x + threadIdx.x;

    // Cargar el vector v en memoria compartida
    // Asumimos que N es menor que el número de hilos por bloque
    if (threadIdx.x < N) {
        shared_v[threadIdx.x] = v[threadIdx.x];
    }
    __syncthreads();  // Sincronizar para asegurar que v se ha cargado completamente

    if (row < M) {
        int sum = 0;
        for (int col = 0; col < N; col++) {
            sum += A[row * N + col] * shared_v[col];
        }
        x[row] = sum;
    }
}
